...
export default class extends Vue {
  ...
  async mounted() {
    ...
    const hexData = this.$route.params.hextxdata;
    console.log("TX DATA:", hexData);

    this.txData = TxMsg.read(new CarbonBinaryReader(hexToBytes(hexData)));
    console.log("TX DATA READ:", this.txData);
  ...
  get txDescription() {
    if (!this.txData) return "No transaction data";
    ...
    const txTypeNames = {
      ...
      2: 'Trade',
      3: 'Transfer Fungible',
      4: 'Transfer Fungible (Gas Payer)',
      5: 'Transfer Non-Fungible Single',
      6: 'Transfer Non-Fungible Single (Gas Payer)',
      7: 'Transfer Non-Fungible Multi',
      8: 'Transfer Non-Fungible Multi (Gas Payer)',
      9: 'Mint Fungible',
      10: 'Burn Fungible',
      11: 'Burn Fungible (Gas Payer)',
      12: 'Mint Non-Fungible',
      13: 'Burn Non-Fungible',
      14: 'Burn Non-Fungible (Gas Payer)',
      15: 'Phantasma',
      16: 'Phantasma Raw'
    };

    const txTypeName = txTypeNames[this.txData.type as keyof typeof txTypeNames] || 'Unknown';
    const txDescription = txTypeName + " transaction";
  ...
  async signtx() {
    ...
    try {
      if (this.requiresManualWif) {
        ...
        hash = await state.signCarbonTxWithPassword(
          txdata,
      ...
      chrome.runtime.sendMessage({
        ...
        tabid,
        sid,
        data: { hash, id: parseInt(id), success: true },
      });
